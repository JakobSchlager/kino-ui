...
export class SeatBookingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.ticketService
      .getAllSeats(this.presentationId)
      .subscribe((x) => (this.reservedSeats = x));
    this.roomService
      .getAllSeats()
      .subscribe((x) =>
        this.initSeats(x.filter((y) => y.roomId == this.roomId))
      );
    this.movieService
      .getOneMovie(this.movieId)
      .subscribe((x) => (this.movie = x));
    this.movieService
      .getPresentationsOfMovie(this.movieId)
      .subscribe(
        (x) =>
          (this.presentation = x.filter((y) => y.id == this.presentationId)[0])
      );
  }
